...
export default function RequestHeader({ data }) {
  const { breakdown, paymentInfo} = data;
  ...
  return (
    <>
      ...
      {/* Desktop layout  Uncomment to implement the mobile layout*/}
      {/* <div className="d-none d-md-flex mb-4"> */}
          ...
          <MDBTableBody>
            ...
            {breakdown.map(p => (
              <tr key={p.date+"-"+p.balance}>
                <td>{p.payment_id? (<LinkCell id={p.payment_id} text={p.payment_id} onClick={() => {openModal(p.payment_id);}}/>): '-'}</td>
                <td>{t(p.type)}</td>
                <td>{p.status_name || '-'}</td>
                <td>{formatDate(p.date, {year: 'numeric', month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit'})}</td>
                {p.amount ? (<td className="text-end" style={Number(p.amount)<=0?{color:"red"}:{color:"green"}}>{Number(p.amount) < 0 ? `-$${Math.abs(Number(p.amount)).toFixed(2)}` : `$${Number(p.amount).toFixed(2)}`}</td>):(<td className="text-end">{'-'}</td>)}
        ...
        />
      {/* </div> */}
      {/* Mobile layout should lives here */}
    </>
  );
}
